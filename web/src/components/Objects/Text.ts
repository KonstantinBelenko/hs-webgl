...
export default class Text2D {
    ...
    public createLabel(content: string, position: THREE.Vector3): void {
        ...
        this.textObject.position.copy(position);

        this.scene.add(this.textObject);
    }

    public updateLabelContent(content: string, location?: THREE.Vector3): void {
        if (this.textObject && this.textObject.element instanceof HTMLDivElement) {
            this.textObject.element.textContent = content;
            if (location) {
                this.textObject.position.copy(location);
            }
        }
    }
}
